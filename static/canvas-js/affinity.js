  ...
  //  Initialize d3 force-directed graph object
{
  ...
  //  Add listener to track mouse position on SVG (needed to position
  //  tooltips when mouse hovers over a node)
  ...
  svg.addEventListener( "mousemove", function( e ) {
    var  off;				// SVG offset on page

    mouse_x( e.pageX );			// Raw mouse position
    mouse_y( e.pageY );

    off = elem_offset( this );		// SVG offset on page

    canvas_x = mouse_x() - off.x;	// Mouse position on SVG
