  ...
  //  blue-green colours) in both sentiment and topic tabs
{
  ...
  for( tab = 0; tab <= 5; tab++ ) {	// For all tabs with legends
    ...
    for( i = 0; i < 25; i++ ) {		// For 25 circles in legend
      ...
      a = 1.0;	// Alpha decreases along circles
  ...
  //  n:   Count to display (default is number of tweets)
{
  ...
  //  element is the tag cloud SVG element
    ...
    vis.selectAll( ".cloud-count" ).remove();

    vis.append( "svg:text" )		// Draw query term
      .attr( "class", "cloud-count" )
      .attr( "x", ( $("#cloud-div").width() - 50 ) + 40 )
      .attr( "y", 10 )
  ...
  //    circles under the mouse pointer
{
  ...
  //  tabs, and heatmap cells on the heatmap tab

  if ( tab_ID() < 0 || tab_ID() > 2 ) {
    return;
  }

  //  Ensure canvas has focus before trying to draw tooltip

  if ( ( tab_ID() == 0 && !tweet_focus() ) ||
       ( tab_ID() == 1 && !topic_focus() ) ) {
  ...
  if ( tab_ID() < 2 ) {			// Tooltips for tweet circles?
    tweet_ID = within_tweet();		// Identify tweets
  ...
  //  term:  Term to inspect
{
  ...
  if ( term.length == 0 ||
       ( ( anew = anew_find_word( term ) ) == -1 &&
         ( anew = anew_find_stem( term ) ) == -1 &&
         ( anew = happiness_find_word( term ) ) == -1 ) ) {
    return col;
  }
