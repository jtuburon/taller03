...
function draw_tweets(tweets_array)
  //  Draw tweets on sentiment canvas
{
  ...
  for( i = 0; i < tweets_array.length; i++ ) {  // For all tweets
    ...
    ctx.fillStyle ="rgba(" + col.r + "," + col.g + "," + col.b + "," + ( 1.0-col.a ) + ")";
    pos = tweet_pos(tweetObj, w, h, "tweet-canvas" );    
    ctx.beginPath();
    ctx.arc( 25 + pos.x, 25 + pos.y, tweetObj.ratio, 0.0, ang, false );
    ctx.stroke();
    ctx.fill();
    ctx.closePath();
  }
