...
var  block_sz = 2;			// Recent tweet pages/block queried
var  max_id;				// Current maximum twitter ID
var  max_pg = block_sz * 4;		// Max queries, MUST BE block_sz units
  ...
  //  allow_dup:  Allow duplicates (default=false)
{
  ...
  var  tw_anew;				// Array of ANEW terms in tweet body
  ...
  tok = tw_body.toLowerCase().split( /[@&\?!\.,\-\+\/\|~'":;^\$\*\(\)\s]+/ );
...
function comp_anew_avg( tw_anew )
  ...
  //
  //  tw_anew:  Array of ANEW terms in tweet
{
  ...
  //  tw_body:  Tweet body
{
  ...
  cur = tw_strip.search( /\W/ );

  while( cur != -1 ) {
    word = tw_strip.substr( 0, cur );
    word_lower = word.toLowerCase();

    if ( ( term = anew_find_word( word_lower ) ) == -1 ) {
      if ( ( term = anew_find_stem( stemmer( word_lower ) ) ) == -1 ) {
        term = happiness_find_word( word_lower );
      }
    }
  ...
  //  tw_body:  Tweet body
{
  ...
  tok = tw_body.split( /\W/ );		// Split body into tokens

  for( i = 0; i < tok.length; i++ ) {	// For all tokens
    if ( tok[ i ].length == 0 ) {	// Skip empty tokens
      continue;
    }

  //  Check for direct word match, if none check for stem match

    word = tok[ i ].toLowerCase();

    if ( ( term = anew_find_word( word ) ) == -1 ) {
      if ( ( term = anew_find_stem( stemmer( word ) ) ) == -1 ) {
        term = happiness_find_word( word );
      }
    }

    if ( term != -1 ) {			// Save ANEW terms
      anew_term.push( term );
    }
  }
  ...
  //  code)
{
  ...
  for( i = 0; i < block_sz; i++ ) {	// Retrieve block of pages
    ...
    .error( function( jqXHR, status, err ) {
      if ( err != "timeout" ) {
        alert( "Error during tweet query\nError: " + err );
      }
  ...
  //  Retrieve a block of pages from Twitter's v1.1 search API
{
  ...
  //  Catch and ignore timeouts, report other errors
    ...
    error: function( jqXHR, status, err ) {
      if ( err != "timeout" ) {
        alert( "Error during tweet v1.1 query\nError: " + err );
      } 
